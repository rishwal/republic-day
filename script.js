...
function getRandomStyles() {
  ...
  const colors = ["#07e207", "#bebcbc94", "#ff7104"];

  // Choose a random color from the options
  const randomColor = colors[random(colors.length)];

  // Extract RGB values for box-shadow
  const rgbValues = randomColor.match(/\d+/g);

  // Calculate slightly darker color for box-shadow
  const shadowColor = rgbValues.map(value => value - 10).join(',');
  ...
  return `
    ...
    color: ${randomColor};
    box-shadow: inset -7px -3px 10px rgba(${shadowColor}, 0.7);
